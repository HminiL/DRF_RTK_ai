...
class Iris(object):
    ...
    def iris_by_tf(self):
        ...
        # self.vo.fname = 'iris_training'
        # iris_df = reader.csv(reader.new_file(self.vo))
        ...
        # plt.savefig(f'{self.vo.context}iris_tt.png')
        ...
        print(f'predictions[:5]의 값 : {tf.nn.softmax(predictions[:5])}')
    ...
    def loss(self, model, x, y, training):
        ...
        # behavior during training versus inference (e.g. Dropout).
        loss_object = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True) # Sparse <-> Dense
    ...
    def base(self):
       ...
       'petal width (cm)', 'species']
        ...
        # print(f'아이리스 y 값 : {y}')
        # Learngin
        clf = RandomForestClassifier(n_jobs=2, random_state=0) # n_jobs epochs와 유사
        clf.fit(train[features], y)
        # print(clf.predict_proba(test[features])[0:10])

        # accuracy
        preds = iris.target_names[clf.predict(test[features])]
        print(f'아이리스 preds 결과 : \n {preds[0:5]}')
        temp = pd.crosstab(test['species'], preds, rownames=['Actual Species'], colnames=['Predicted Species'])
        print(f'아이리스 crosstab 결과 : \n {temp}')
        '''
