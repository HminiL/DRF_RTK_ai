...
class Crime():
    ...
    def process(self):
        ...
        print('[8] Seoul Map folium CREATION ')
        vo.fname = 'geo_simple'
        state_geo = reader.json(reader.new_file(vo))
        crime_map = folium.Map(location=[37.566345, 126.977893], zoom_start=12, title='Stamen Toner')
        ...
        for i in crime_df.index:
            folium.CircleMarker([crime_df['lat'][i], crime_df['lng'][i]],
                                ...
                                fill_color='#0a0a32').add_to(crime_map)

        crime_map.save(vo.context+'new_data/folium_seoul.html')
    ...
    def crime_police(self, crime_df, reader, vo):
        ...
        # crime_df.to_csv(vo.context+'new_data/crime_police_mer.csv')

        dt = dict(zip(station_lats, station_lngs))
        print(dt)
        with open(vo.context+'new_data/crime_police_lat_lng.csv', 'w', encoding='UTF-8') as f:
            w = csv.writer(f)
            w.writerow(dt.keys())
